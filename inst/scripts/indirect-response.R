...
#{
  ...
  simtag   <- "indirect-response" # script version number
  ...
  obs.times     <- c(0.5,1,2,4,8,12,16,20,24,30,39,48)
  
  xend.typ      <- 700  # User provided x value to max. simulate out 
  ...
  output.typ  <- as.data.frame(long.out) %>%
    filter(!(time==0 & CENT==0)) %>%      # Only Dose rec
