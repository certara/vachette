...
#sim.oral.two.cov <- function(nsim.indiv,iiv,ruv,nvpc,SAVE=F,PROP=T)
#{
  my.model <- mcode("oral1cmt.wt.age", oral1cmt.wt.age)
  simtag   <- "oral-two-cov" # script version number
  
  # ----------- Model settings ------------------
  
  obs.times     <- c(0.5,1,2,4,6,8,12,18,24,36,48)
  
  xend.typ      <- 750  # User provided x value to max. simulate out 
  ...
  data <- expand.idata(ID=1,WT=c(wt1,wt2),AGE=c(age1,age2))
  ...
  long.out <- my.model %>%
    ...
    idata_set(data) %>%
    carry_out(ADDL, II, EVID) %>%
    mrgsim(delta=mrgdelta,end=xend.typ) 
  
  output.typ  <- as.data.frame(long.out) %>%
    filter(!(time==0 & GUT==0)) %>%     # Only Dose rec at time=0
    mutate(x=time, y=CP) %>%       # Standardize
    mutate(PRED=CP) %>% 
    mutate(vachette.cov1 = WT) %>% 
    mutate(vachette.cov2 = AGE) %>% 
    # Dose number
    mutate(dosenr = 1,
           ID = ID + 1000000) %>%
    dplyr::select(ID, time, EVID, PRED, WT, AGE, EVID) %>%
 ...
 # simulate ipred
  ...
  indivsam.all <- imy.ipred %>%
    ...
    # Dose number
    mutate(dosenr = 1) %>%
    dplyr::select(ID, time, DV, WT, AGE, EVID) %>%
    arrange(ID, time)	   
  ...
  if(nvpc>0)
  {
    ...
    for(ivpc in c(1:nvpc))
    {
      ...
      indivsam.vpc <- rbindlist(list(indivsam.vpc, 
                            indivsam.ivpc %>% dplyr::select(REP,ID,time,DV,WT,AGE,EVID))) %>%
        arrange(REP, ID, time)
    }
  }
